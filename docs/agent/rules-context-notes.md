...
| `working-principles.mdc` | 실행 계획·**분담 임계치(SSOT)**·HUMAN·DoD·커밋 안전·게이트 경계·조사·소통·실패 대응·DB 운영 기본값 (**always**) |
| `20-web-vs-app.mdc` | 웹 테이블/명시 탐색 vs 앱 스크롤·리스트 (**globs / UI 시**, always 아님) |
| `30-table-pagination.mdc` | 테이블 필터·기본 15건(기존·PRD 우선)·하단 페이지네이션 (**globs / 목록 UI 시**) |
| `40-dark-mode.mdc` | 다크/라이트·토큰·대비·전환 유지 (**globs / UI 시**) |
| `50-index-css-contract.mdc` | 전역 스타일·Stitch 등 합의 (**globs / UI·CSS 시**) |
