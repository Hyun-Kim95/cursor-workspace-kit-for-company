...
- 고객 프로젝트형 게이트/승인 흐름은 `.cursor/rules/60-delivery-gates.mdc`를 기본으로 하고, `.cursor/rules/70-client-lifecycle-default.mdc`는 **고객 E2E 명시(opt-in)** 시에만 lifecycle을 강제한다. 전환 체크리스트: [`docs/agent/company-kit-checklist.md`](docs/agent/company-kit-checklist.md).
...
- 계획·PRD·범위 정리 시 수익·사업자 전제는 `shared/rules/product-monetization-default.mdc`와 `docs/agent/product-assumptions.md`를 따른다(기본: 회사·법인, 광고·후원 비기본). 회사 킷 프로파일: `docs/agent/company-profile.md`.
...
3. `.cursor/rules/70-client-lifecycle-default.mdc`가 **opt-in으로 적용된** 고객 프로젝트 흐름 중 스킬이 정한 **HUMAN·승인·멈춤** 구간
...
- `project-kit/.cursor/rules/60-delivery-gates.mdc`, `70-client-lifecycle-default.mdc` (Gate 유지 · 70은 회사 킷에서 opt-in)
...
- 디자인 승인과 구현 착수 승인 통합 규칙은 `.cursor/rules/70-client-lifecycle-default.mdc`를 따른다(**고객 E2E opt-in**일 때). 사내 기본은 Gate 1 + 디자인/범위 합의 후 `start-feature`.
- 고객 E2E(opt-in)에서 **디자인 선택 후**는 [`docs/qa/stage3-entry-checklist.md`](docs/qa/stage3-entry-checklist.md) → Gate 2 → **ATDD-lite RED** → 구현(`parallel-delivery` / `start-feature`)이며, **선택 후 mock-only 재목업은 기본 금지**(예외는 사용자 명시·문서 기록).
...
- **회사 킷(사내 제품):** 고객 E2E는 기본 진입이 **아니다**. `.cursor/rules/70-client-lifecycle-default.mdc`는 **명시 opt-in**일 때만 `client-project-lifecycle`을 적용한다. 일상 사내 작업은 아래 `plan-feature` / `start-feature` / Gate를 우선한다. 프로파일: [`docs/agent/company-profile.md`](docs/agent/company-profile.md).
- 사용자가 **고객사 납품·고객 E2E**를 명시한 경우에만 `client-project-lifecycle`을 따른다(PRD·디자인 등 HUMAN 구간에서 멈춤). 그 경우 **디자인 승인 = 구현 착수 승인**이며 구현 시작 중복 승인을 추가로 요구하지 않는다. 다축 검증·리뷰어 GATE는 스킬 **단계 4B~4D(선택)** 및 `docs/qa/reviewer-gate-rubric.md`를 참고한다.
...
- 고객 E2E(opt-in) **단계 2**에서는 `design-brief`(단계 2-0) 완성 전 이중 목업(2A/2B)을 시작하지 않는다. 그 외 랜딩·마케팅 목업도 `design-brief`를 우선 고려한다. (`design-system-agent` Owner)
